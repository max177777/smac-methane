...
from utils.data_loader import (
    COUNTRY_META, COUNTRY_ORDER, country_monthly, CURRENT_YEAR, DATA_RANGE_LABEL,
...
with col_h1:
    ...
    st.markdown(f'<div class="smac-meta">monthly subnational methane · climate trace {DATA_RANGE_LABEL}</div>',
...
with kpi_cols[0]:
    st.metric(f"{CURRENT_YEAR} Total CH₄{' · ' + loc if has_loc else ''}", f"{fmt_mt(y_now)} Mt",
...
with kpi_cols[3]:
    if has_loc:
        ...
        if len(loc_row):
            ...
            st.metric("Share of national total", "—", f"no {CURRENT_YEAR} data for this unit", delta_color="off")
    else:
...
with col_t:
    ...
    st.markdown(
        f"<h3>Monthly CH₄ · tonnes · {loc if has_loc else 'all SMAC subnational units'}</h3>",
        unsafe_allow_html=True,
    )
    recent_label = f"{CURRENT_YEAR - 1}–{CURRENT_YEAR}"
    period = st.radio("Period", options=["All years", recent_label],
    ...
    if period == recent_label:
        monthly = monthly[monthly["year"] >= CURRENT_YEAR - 1]
    st.plotly_chart(time_series_plotly(monthly, height=320), use_container_width=True,
                    config={"displayModeBar": False})

with col_g:
    eyebrow("CH₄ vs CO₂e")
    st.markdown(f"<h3>IPCC AR6 · GWP100 vs GWP20 · {CURRENT_YEAR}</h3>", unsafe_allow_html=True)
    top3 = ranking.head(3)
    st.markdown(
...
st.markdown(f"<h3>{CURRENT_YEAR} ranking · share of national CH₄</h3>", unsafe_allow_html=True)
...
if not selected_labels:
    ...
    for label in selected_labels:
        ...
        y_now_c = float(m[m["year"] == CURRENT_YEAR]["ch4_tonnes"].sum())
        y_prior_c = float(m[m["year"] == CURRENT_YEAR - 1]["ch4_tonnes"].sum())
        peak = m.loc[m["ch4_tonnes"].idxmax()]
        table_rows.append({
            "Region": loc,
            "Country": COUNTRY_META[c_iso]["name"],
            f"{CURRENT_YEAR} CH₄ (t)": y_now_c,
            "GWP100 CO₂e (t)": y_now_c * GWP100,
            "GWP20 CO₂e (t)": y_now_c * GWP20,
            f"YoY {prior_yy}→{now_yy} (%)": pct_change(y_now_c, y_prior_c),
    ...
    if parts:
        ...
        cmp_table = pd.DataFrame(table_rows).sort_values(f"{CURRENT_YEAR} CH₄ (t)", ascending=False)
        st.dataframe(
            cmp_table,
            hide_index=True,
            use_container_width=True,
        ...
        if not has_sector_data():
            ...
            with sec_c1:
                sec_year = st.selectbox("Sector year", [CURRENT_YEAR, CURRENT_YEAR - 1, CURRENT_YEAR - 2, CURRENT_YEAR - 3, CURRENT_YEAR - 4],
...
st.markdown(f"<h3>{CURRENT_YEAR} totals across the SMAC</h3>", unsafe_allow_html=True)

# Build a cross-country dataframe
rows = []
for c in COUNTRY_ORDER:
    cy = country_yearly(c)
    y_now_x = float(cy[cy["year"] == CURRENT_YEAR]["ch4_tonnes"].iloc[0]) if CURRENT_YEAR in cy["year"].values else 0
    y_prev_x = float(cy[cy["year"] == CURRENT_YEAR - 1]["ch4_tonnes"].iloc[0]) if (CURRENT_YEAR - 1) in cy["year"].values else 0
    rows.append({
        "Country": COUNTRY_META[c]["name"],
        "iso": c,
        "ch4_mt": y_now_x / 1e6,
        "yoy": pct_change(y_now_x, y_prev_x),
    })
...
bar = alt.Chart(cross_df).mark_bar(height=22).encode(
    y=alt.Y("Country:N", sort="-x",
            axis=alt.Axis(title=None, labelFontSize=12, labelFont="Inter",
                          labelColor=INK)),
    x=alt.X("ch4_mt:Q",
            axis=alt.Axis(title=f"{CURRENT_YEAR} CH₄ (Mt)", titleFontSize=10,
